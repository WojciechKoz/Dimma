...
# prepare dataset
FS_DARK = 'data/fs_dark/'

def create_dataset(split):
    # load images
    fsd_images = list(sorted(Path(f'{FS_DARK}/{split}/high/').glob('*.png'), key=lambda x: int(x.stem)))
    print(fsd_images[START_IDX:START_IDX+TRAIN_SIZE])

    def load_pair(idx):
        light = cv2.imread(str(fsd_images[idx]))[..., ::-1]
        dark = cv2.imread(f'{FS_DARK}/{split}/low/{fsd_images[idx].name}')[..., ::-1]
    ...
    for i in tqdm(range(START_IDX, START_IDX+TRAIN_SIZE)):
        ...
        if x_.shape[0] > x_.shape[1]: # we can transpose because mdn treats every pixel independently
            x_ = x_.transpose(1, 0, 2)
            y_ = y_.transpose(1, 0, 2)
        x.append(x_)
        y.append(y_)
...
dl = create_dataset('train')
dl_val = create_dataset('val')
...
def test_model(model):
    # read image
    VAL_PATH = 'data/fs_dark/val/'
    l = cv2.imread(VAL_PATH + 'high/1.png')[..., ::-1] / 255.0
    d = cv2.imread(VAL_PATH + 'low/1.png')[..., ::-1] / 255.0
...
def train(model):
    ...
    for epoch in range(1, 1001):
        ...
        for i, (x, y) in enumerate(dl):
            rand_x = randint(0, x.shape[3] - 200)
            rand_y = randint(0, x.shape[2] - 200)
        ...
        if epoch % 100 == 0:
            ...
            val_losses = []
            for x, y in dl_val:
                val_losses.append(model.loss(x.to(device), y.to(device)).cpu().item())
            print(f'Epoch {epoch:3d} | Train loss: {np.array(losses).mean():.4f} | Test loss: {np.array(val_losses).mean():.4f}')
...
def estimate_luminance_mapping():
    fsd_images = list(sorted(Path(f'{FS_DARK}/train/high/').glob('*.png'), key=lambda x: int(x.stem)))

    def load_pair(idx):
        light = cv2.imread(str(fsd_images[idx]))[..., ::-1]
        dark = cv2.imread(f'{FS_DARK}/train/low/{fsd_images[idx].name}')[..., ::-1]
...
# create folder
Path(f'./dimmers/fsd/{TRAIN_SIZE}shot{POSTFIX}').mkdir(parents=True, exist_ok=True)

# save model 
torch.save(model.cpu().state_dict(), f'./dimmers/fsd/{TRAIN_SIZE}shot{POSTFIX}/mdn.pt')
np.save(f'./dimmers/fsd/{TRAIN_SIZE}shot{POSTFIX}/mean.npy', mean)
np.save(f'./dimmers/fsd/{TRAIN_SIZE}shot{POSTFIX}/std.npy', std)